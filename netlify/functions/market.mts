...
export default async (req: Request, _context: Context) => {
  ...
  if (req.method === "GET") {
    ...
    if (itemName) {
      // Drift is computed ONLY here — the moment someone actually opens a
      // specific blade's detail view. Nothing else touches the market for
      // this item, so a rarely-viewed rare blade can go untouched (and
      // uncharged in compute) for as long as nobody looks at it.
    ...
    // Bulk listing: a single cheap cached read, no per-item work at all.
    // Grid prices reflect whatever they were the last time each specific
    // item was individually opened — real activity, not a background loop.
